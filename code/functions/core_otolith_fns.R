...
stratified_samp_10cm <- function(collect,
                                 bootstrap_hauls_,
                                 lengths_) {
  ...
  ## Convert lengths in cm to 10cms
  lengths_$LENGTH_10CM <- floor(lengths_$LENGTH / 100)
  ...
  total_by_haul <- data.frame(CRUISE = bootstrap_hauls_$CRUISE,
                              HAULJOIN = bootstrap_hauls_$HAULJOIN,
                              FREQUENCY = bootstrap_otos)
  return(list(total_by_cruise = total_by_cruise,
              total_by_haul = total_by_haul))
}
